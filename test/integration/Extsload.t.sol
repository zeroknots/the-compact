...
/**
 * @title ExtsloadTest
 * @notice Integration test for the Extsload functionality in TheCompact, which provides external functions
 * for reading values from storage or transient storage directly.
 */
contract ExtsloadTest is Setup {
     ...
     * This test verifies that we can correctly read an allocator address from storage
     ...
     * This test verifies that we can correctly read multiple allocator addresses from storage.
     */
    ...
    function test_exttload() public {
        ...
        try theCompact.exttload(bytes32(0)) returns (bytes32) {
            // If we get here, transient storage is supported

            // We can't directly write to transient storage from the test,
            // so we'll just verify that the function doesn't revert
            bytes32 slot = bytes32(uint256(1));
            bytes32 value = theCompact.exttload(slot);

            // The value should be 0 since we haven't written to this slot
     ...
     * This test verifies that we get the expected zero address when reading a storage slot
    ...
    function test_exttload_reentrancyGuard() public {
        ...
        // Create signatures
        {
            ...
            // For the allocator signature, we'll use a dummy value since our ReentrantAllocator
            // will check the reentrancy guard during authorizeClaim regardless of the signature
